...
コミット・履歴操作の依頼では、ハーネス既定の git 手順（git add / git commit / git status での確認）
ではなく、本ファイルの jj 手順を用いること。
...
| `claude` | `~/.claude/` | 完了 |
| `zsh` | `~/.zshrc` 等 | 完了 |
| `sheldon` | `~/.config/sheldon/` | 完了 |
| `git` | `~/.gitconfig` + `~/.config/git/ignore` | 完了 |
| `brew` | `~/.Brewfile` | 完了 |
| `codex` | `~/.codex/` | 完了（template-only） |
| `ghostty` | `~/.config/ghostty/` | 完了 |
| `alacritty` | `~/.config/alacritty/` | 完了 |
| `aerospace` | `~/.config/aerospace/` + `~/.local/bin/` | 完了 |
| `sketchybar` | `~/.config/sketchybar/` | 完了 |
| `borders` | `~/.config/borders/` | 完了 |
| `tmux` | `~/.config/tmux/` | 完了 |
| `herdr` | `~/.config/herdr/`（config.toml + scripts のみ） | 完了 |
| `yazi` | `~/.config/yazi/` | 完了 |
| `nvim` | `~/.config/nvim/` | 完了（subtree merge） |
| `raycast` | `~/.config/raycast/` | 完了 |
| `karabiner` | `~/.config/karabiner/` | 完了 |
...
## シェルスクリプトの互換性（macOS default bash 3.2）

macOS default の `/bin/bash` は 3.2 系で固定されている（Apple は GPLv3 回避のため更新停止）。dotfiles 配下のスクリプトは bash 3.2 でも動作すること。

### 禁止 / 回避する構文

| NG | 代替 |
|----|------|
| `declare -A arr`（連想配列、bash 4.0+） | 通常配列 + key prefix / 別変数 |
| `local -n ref=$1`（nameref、bash 4.3+） | `${!var}` による間接参照 |
