# PRP: Obsidianリンク ホバープレビュー

## 概要

`[[wikilink]]`形式のObsidianリンク上でカーソルを500ms置くと、リンク先ファイルの内容をフローティングウィンドウでプレビュー表示する機能。

## ゴール

- **ホバー500ms**: リンク上にカーソルを500ms置くとプレビュー表示
- **フォーカス移動**: プレビュー表示後、プレビューウィンドウにフォーカス
- **通常操作**: プレビュー内でj/k/gg/G/検索など通常のVim操作が可能
- **閉じる操作**: ZZ/ZQ/q/Escで閉じる
...
`CursorHold`イベント + フローティングウィンドウ（`nvim_open_win`）+ フォーカス移動

### 参考実装

- `lua/user-plugins/pending-tasks.lua` - フローティングウィンドウ実装パターン

---

## 実装詳細

### 新規ファイル: `lua/user-plugins/obsidian-hover-preview.lua`
...
local function setup_preview_keymaps(buf)
  ...
  -- ZZ/ZQ/q/Esc で閉じる
  vim.keymap.set("n", "ZZ", function() M.close_preview() end, opts)
  vim.keymap.set("n", "ZQ", function() M.close_preview() end, opts)
  vim.keymap.set("n", "q", function() M.close_preview() end, opts)
  vim.keymap.set("n", "<Esc>", function() M.close_preview() end, opts)

  -- gf でリンク先を実際に開く（プレビューから本編集へ）
  vim.keymap.set("n", "gf", function()
    local file_path = vim.b[buf].preview_file_path
    M.close_preview()
    if file_path then
...
function M.show_preview(file_path)
  ...
  -- ファイルパスをバッファ変数に保存（gfで使用）
  vim.b[buf] = { preview_file_path = file_path }
...
```lua
-- lua/user-plugins/obsidian-hover-preview.lua を有効化
require("user-plugins.obsidian-hover-preview").setup({
  delay = 500,        -- 500msホバー
  max_height = 15,    -- 最大15行
  preview_lines = 50, -- 50行まで読み込み
})
```
...
```
1. Markdownファイル内で[[wikilink]]にカーソルを置く
   ↓
2. 500ms経過（カーソル静止）
   ↓
3. プレビューウィンドウ表示
   ↓
4. フォーカスがプレビューウィンドウに移動
   ↓
5. 通常のVim操作が可能
   - j/k/gg/G: スクロール
   - /: 検索
...
|-----------|-----------|------|
| `delay` | 500 | プレビュー表示までの遅延（ms） |
| `max_width` | 80 | プレビューウィンドウ最大幅 |
| `max_height` | 20 | プレビューウィンドウ最大高さ |
| `border` | "rounded" | ボーダースタイル |
| `preview_lines` | 50 | 読み込む最大行数 |
...
- `obsidian.nvim` - リンク解決に使用

## リスク

- **中**: カスタム実装のため動作確認が必要
- **パフォーマンス**: CursorMoved頻発時のデバウンスで対応済み
...
- [ ] `[[存在するリンク]]`上で500msホバー → プレビュー表示
- [ ] プレビュー表示後、フォーカスがプレビューウィンドウに移動
- [ ] j/k でスクロール可能
- [ ] `/` で検索可能
- [ ] ZZ で閉じる → 元のウィンドウに戻る
- [ ] ZQ で閉じる → 元のウィンドウに戻る
...
- [ ] Esc で閉じる → 元のウィンドウに戻る
- [ ] gf でファイルを本編集で開く
- [ ] `[[存在しないリンク]]`上でホバー → 何も起きない
- [ ] リンク外でホバー → 何も起きない
- [ ] プレビュー内容がMarkdownとしてハイライトされる
