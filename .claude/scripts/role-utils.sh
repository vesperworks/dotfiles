...
# 
# このファイルは worktree を使わないシンプルなワークフローをサポートします。
# すべての中間成果物は ./tmp/ ディレクトリに保存されます。
...
switch_role() {
    ...
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    log_role "🎭 Switching to $new_role mode"
...
show_role_description() {
    ...
    case "$role" in
        "Explorer")
            echo "🔍 調査・探索: 既存コードの理解、要件の明確化、制約の特定"
            ;;
        "Analyst")
            echo "📊 分析・評価: 影響範囲の分析、リスク評価、実装戦略の検討"
            ;;
        "Designer")
            echo "🎨 設計・計画: アーキテクチャ設計、インターフェース定義、テスト戦略"
            ;;
        "Developer")
            echo "💻 実装・開発: コード実装、ユニットテスト作成、段階的コミット"
            ;;
        "Reviewer")
            echo "✅ レビュー・品質確認: コードレビュー、テスト実行、ドキュメント整備"
            ;;
        *)
...
select_next_role() {
    ...
    echo "1) Explorer  - 調査・探索"
    echo "2) Analyst   - 分析・評価"
    echo "3) Designer  - 設計・計画"
    echo "4) Developer - 実装・開発"
    echo "5) Reviewer  - レビュー・品質確認"
    echo "6) Exit      - 終了"
    echo ""
