...
### 中間成果物（.brain/）
- **目的**: エージェントの中間成果物・一時ファイル
- **形式**: `.brain/{category}/{timestamp}-{name}.md`
- **gitignore**: 必須（コミットしない）

### 知見・ドキュメント（thoughts/）
- **目的**: リサーチ結果・知見・決定事項の永続化
- **形式**: `thoughts/{shared,notes}/{date}-{topic}.md`
- **gitignore**: 対象外（コミットする）

### テストファイル
- **実行結果**: `.brain/test/` に保存
- **テストスクリプト**: `src/test/` に配置（特に指定がなければ）
