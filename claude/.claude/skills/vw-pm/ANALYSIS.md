...
while IFS= read -r repo; do
  [[ -z "$repo" ]] && continue
  page=$(gh issue list --repo "$repo" --state all --limit 100 \
    --json number,title,labels,state \
    | jq --arg r "$repo" '[.[] | . + {repo: $r}]')
  ALL_ISSUES=$(jq -n --argjson a "$ALL_ISSUES" --argjson b "$page" '$a + $b')
done < <(echo "$SCOPE_REPOS" | jq -r '.[]')
```

各 Issue に `repo` フィールドを付与しておく（後段で repo 別集計が必要なため）。
