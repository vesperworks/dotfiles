...
Cascade iteration from a parent issue to all its sub-issues.
...
Examples:
  # Cascade iteration from issue #10 to its sub-issues
  $0 10 --project 1 --owner @me

  # Dry run to see what would happen
  $0 10 --project 1 --owner @me --dry-run

  # Specify repository explicitly
  $0 10 --project 1 --owner @me --repo owner/repo
EOF
  exit 1
}
...
# Step 4: Get sub-issues
echo "Cascading to sub-issues..."
SUB_ISSUES=$(gh api "repos/$REPO/issues/$PARENT_ISSUE/sub_issues" --jq '.[].number' 2>/dev/null || echo "")
...
for sub_issue in $SUB_ISSUES; do
  ...
  sub_issue_title=$(echo "$sub_iteration_json" | jq -r '.issueTitle // empty')
...
echo "───────────────────────────────────────────────"
echo "  Parent: #$PARENT_ISSUE ($PARENT_ITERATION_TITLE)"
echo "  Updated: $updated_count sub-issue(s)"
echo "  Skipped: $skipped_count sub-issue(s)"
echo "═══════════════════════════════════════════════"
