...
- **inbox が積み上がっても回り続ける** → verification debt が複利で積もる（一晩20 PR問題）。inbox 滞留上限（既定10件）超過でループ停止を設計に含める（backpressure）
...
実行主体は script（決定論: 列挙・変換・ゲート・コミット）/ LLM（判断・生成）を明記する。
決定論で解けるものは LLM に渡さない。

| ムーブ | 実行主体 | 設計 |
|--------|---------|------|
| Discovery | {script / LLM / script+LLM} | {何を読んで今ターンの仕事を見つけるか。コマンド実物} |
| Handoff | {script / LLM} | {隔離単位。worktree 要否} |
| Verification | {script / LLM / 人間} | {誰が「No」と言うか。Generator と別であること。完了判定に LLM 判断が入るなら stop-judge（別モデル）も明記} |
| Persistence | {script / LLM} | {状態ファイルのパスと1行のフォーマット。既定: リポジトリルートの `.brain/{project}/loop-state/{slug}.md`} |
| Scheduling | {script} | {何が次のターンを回すか} |
...
1. **Stage 1**: 1件を end-to-end で処理する（並列なし・最小対象）
2. **Stage 2**: Evaluator が本物のミスを reject した実績を確認する（reject 実績ゼロのまま拡大しない — 数百ターン reject 0% は検証が機能していない統計的証拠）
3. **Stage 3**: 並列化・対象拡大（worker 数は budget guard でスケール制御）

※ 並列・拡大の予定がない設計は「Stage 1 のみ（拡大予定なし）」と明記して省略可。

## 運用指標（ループ自体の評価）

- Evaluator reject 率: {記録先。状態ファイルに verdict を追記する等}
- 人間介入率（inbox 行き比率）: {記録先}
- 判定: reject 率 0% が続く → Nodding 疑い。介入率が高止まり → 事前チェックの強化を検討
...
| 決定論境界（script/LLM 線引き） | {OK/対策} |
| Stop-judge（該当時） | {OK/対策/非該当} |
| Backpressure（該当時） | {OK/対策/非該当} |
| 段階導入（該当時） | {OK/対策/非該当} |

## コスト上限

- token cap: {per-run / daily} / 最大リトライ: {N}
- inbox 滞留上限（HITL 非同期型のみ）: {N 件。既定 10}
