...
- All interactive elements removed: `.toggle`, `.hud`, `.picker`, `.guides`, `<script>`
...
2. Copy the **toggle button + HUD + picker HTML** (between `</head>` and the first `<section>`)
...
5. Add the overlay (same content box) + toggle + optical-alignment JS (already in the scaffold; point its selector list at your display elements).
