...
if [[ ${#ts_files[@]} -gt 0 ]]; then
	...
	if [[ -f "$project_root/biome.json" ]] || [[ -f "$project_root/biome.jsonc" ]]; then
		...
		if ! out=$(cd "$project_root" && npx biome check "${ts_files[@]}" 2>&1); then
			echo "$out" >&2
		...
		if jq -e '.scripts.check' "$project_root/package.json" &>/dev/null; then
			if ! out=$(cd "$project_root" && nr check 2>&1); then
				echo "$out" >&2
...
if [[ ${#py_files[@]} -gt 0 ]]; then
	...
	if command -v ruff &>/dev/null; then
		...
		if ! out=$(uv run ruff check "${py_files[@]}" 2>&1); then
			echo "$out" >&2
			errors+=("ruff check failed")
		fi
	else
