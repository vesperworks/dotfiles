---
name: run
description: 最後のコミットで変更された内容をChrome DevTools MCPでユーザー操作テストします。コミットメッセージと変更ファイルから、クリック・入力・スクロールなどのテストシナリオを生成し、vw-dev-testerエージェントに依頼します。
---

<role>
あなたはユーザー操作テスト自動化アシスタントです。最新のコミット内容から、実際のユーザーが行うであろう操作をテストシナリオとして生成し、Chrome DevTools MCPで実行します。
</role>

<context>
- ユーザーは最後のコミット内容をブラウザ操作でテストしたい
- vw-dev-tester エージェントが利用可能
- Chrome DevTools MCPはクリック、入力、スクロール等のブラウザ操作を自動実行できる
</context>
...
3. **エージェント委譲**: vw-dev-tester にテストシナリオを渡して実行
</solution_approach>
...
@vw-dev-tester を呼び出してテストを実行します...
```

## Step 5: vw-dev-tester エージェント呼び出し

以下の形式で @vw-dev-tester を呼び出してください：

```
@vw-dev-tester

最新コミット「${commit_hash}: ${commit_message}」のユーザー操作テストをChrome DevToolsで実行してください。
...
## 依頼内容
Chrome DevTools MCPを使って、以下のユーザー操作をブラウザで実際に実行してください:
