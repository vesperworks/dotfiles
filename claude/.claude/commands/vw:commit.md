...
1. `jj diff --name-only` で変更ファイル一覧を取得（ステージング概念なし）
   > **⚠️ 混入チェック（必須）**: 一覧に「このセッションで自分が触っていないファイル」が
   > 含まれていたら停止。並行セッションの編集がワーキングコピーに自動スナップショット
   > されている可能性が高い。ユーザーに提示して切り分けてから split に進む
2. `jj diff` で変更内容を確認
...
### Step 4.5: ローカル品質ゲート（push / PR 前は必須）

コミット後にリモート push する（PR を作る）場合、push 前に必ずローカルで
品質ゲートを通す。「CI で初めて落とす」手戻りを防ぐ。

1. 変更スコープに応じてテストを実行:
   - 特定パッケージのみの変更 → そのパッケージのテストを最小実行
     例: `pnpm --filter <package> exec vitest run <変更/関連テストファイル>`
   - co-located テスト方針の場合、変更ファイルの隣の `*.test.ts(x)` を優先
   - 横断的変更 → プロジェクトの品質コマンド（例: `pnpm lint && pnpm test`）
2. fail があれば修正してから push。green を確認後に push / PR に進む
3. ローカル実行が重い / 不可能な場合は、その旨をユーザーに伝えてから push する
...
### Known failure modes（jj）

- `jj squash` が返ってこない → 対話エディタ待ち。`--use-destination-message` を付けて回避
- `jj split` 後にさらに編集してから次の split をすると、新編集が混入する
  （split は対象スコープの全編集完了後にのみ実行。feedback-jj-split-after-all-edits 参照）
- `vw:commit.md` のような `:` 入りパスは fileset として `'"path"'` の二重クォートで渡す
- サンドボックスで `jj log`/`jj status` が「Failed to take lock」になったら
  `jj --ignore-working-copy log` で読み取る（書き込み系は pbcopy でユーザーへ）
...
- **push して PR を作る場合は、push 前にローカルで lint/test を通す（CI で初めて落とさない）**
