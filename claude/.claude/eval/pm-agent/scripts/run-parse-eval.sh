...
*)
	echo "エラー: 第1引数は 'old', 'new', 'split', 'rules', または 'fewshot' を指定してください"
	exit 1
	;;
esac
...
PROMPT_TEMPLATE='vw-pm スキルを使って、以下の議事録を4層構造に分類してください。
...
for case_id in "${CASE_ARRAY[@]}"; do
	...
	for run in $(seq 1 "$RUNS"); do
		...
		# --allowedTools は Skill(vw-pm) のみ。Bash は許可しない = gh コマンド実行不可
		claude -p "${PROMPT_TEMPLATE}${case_input}" \
			--allowedTools "Skill(vw-pm)" \
