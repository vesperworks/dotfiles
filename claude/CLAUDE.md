...
- **MCP Integration**: Context7, Chrome DevTools
...
- `vw-pm` SKILL（`/vw-pm [meeting_notes]`）: GitHub Projects PM Agent（議事録→タスク作成、Projects管理）
- `/vw:run`: Chrome DevTools MCPでユーザー操作テスト
- `vw-research` SKILL（`/vw-research`）: 対話型リサーチアシスタント（壁打ち・インタビュー・包括的調査）
- `vw-note` SKILL（`/vw-note [term]`）: 技術用語の解説・記録（Atomic Notes形式）
...
- **vw-dev-tester**: E2Eテスト（Chrome DevTools MCP）
...
#### ユーティリティエージェント（vw-file-summarizer: 1）
- **vw-file-summarizer**: vw:index SKILL 用の Markdown 一括要約（mtime キャッシュ付き）
