...
# calls so the 30s-tick bar update stays cheap.
get_ssid() {
  local age mtime now ssid
  if [ -f "$SSID_CACHE" ]; then
    mtime=$(stat -f %m "$SSID_CACHE" 2>/dev/null || echo 0)
    now=$(date +%s)
    age=$((now - mtime))
  else
    age=$((SSID_CACHE_AGE + 1))
  fi

  if [ "$age" -gt "$SSID_CACHE_AGE" ]; then
